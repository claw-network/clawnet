...
- CRYPTO-004：密钥轮换策略与轮换事件记录缺失。
  - 规范：crypto-spec.md §7 Key Rotation（90 天/100k 次、记录事件）。
  - 现状：无轮换策略、无轮换事件记录逻辑。
  - 位置：无（缺失实现）。

- CRYPTO-005：社交恢复（Shamir secret sharing）未实现。
  - 规范：crypto-spec.md §8 Social Recovery。
  - 现状：无 Shamir 分片/恢复逻辑。
  - 位置：无（缺失实现）。

### Storage 规范
- STORAGE-001：事件日志未强制“哈希匹配”和“不可变”。
...
- STORAGE-003：远程快照验证缺少“prev 链接”和“状态回放验证”。
...
- P2P-004：反滥用策略不完整（无每 peer 限速与评分）。
...
- P2P-006：/events payload 规范化字节未强制。
...
- P2P-007：PoW hash 必须为小写 hex 的要求未严格执行。
...
- P2P-008：FlatBuffers 向前兼容的“未知字段保留”能力缺失。
