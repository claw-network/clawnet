...
└────┬─────┘    └────┬─────┘    └────┬─────┘
     ...
     ▼               ▼               ▼
┌──────────┐    ┌──────────┐    ┌──────────┐
│  Geth V1 │◄──►│  Geth V2 │◄──►│  Geth V3 │    ← 第一层：区块链层 (Geth P2P :30303)
└──────────┘    └──────────┘    └──────────┘
```
...
| 组件 | 说明 |
|------|------|
| **Geth** (Go-Ethereum) | 以太坊官方客户端的 Go 语言实现，负责运行 EVM、维护区块链状态、参与共识出块、提供 JSON-RPC API |
| **Clique PoA** | Proof of Authority 共识算法，3 个预授权 Validator 节点轮流出块，出块间隔 2 秒 |
...
| **ethers.js** | Node.js 内的以太坊库，编码/签名/发送交易到 Geth |
...
if (this.config.chain) {
  ...
  // chain.rpcUrl → Geth 的 JSON-RPC 端点
}
```
...
```typescript
{
  rpcUrl: "https://rpc.clawnetd.com",   // Geth 端点
  chainId: 7625,
      ...
      → DaoService.vote()
        → ContractProvider.dao.vote(proposalId, support)
          → ethers.js 编码 calldata + 签名交易
            → HTTP POST → Geth JSON-RPC (eth_sendRawTransaction)
              → Geth 打包进区块 → ClawDAO.sol 合约执行
            ← 返回交易回执 (receipt)
          ← 解析 receipt
        ← 返回 VoteCastResult
      ← 返回 JSON 响应
...
| **Geth** | 运行时 | testnet/mainnet 的真实区块链节点 |
...
| clawnetDevnet | 7625 | 本地 Geth | `http://127.0.0.1:8545` | 开发调试 |
| clawnetTestnet | 7625 | 3 节点 Geth PoA | `https://rpc.clawnetd.com` | 测试网 |
