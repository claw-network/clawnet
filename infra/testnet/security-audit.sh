...
#   ./security-audit.sh --server A         # audit only Server A
#
...
# Server IPs
SERVER_A="${CLAWNET_SERVER_A:-66.94.125.242}"
SERVER_B="${CLAWNET_SERVER_B:-85.239.236.49}"
SERVER_C="${CLAWNET_SERVER_C:-85.239.235.67}"
...
main() {
  ...
  if [[ "$MODE" == "server" ]]; then
    ...
    case "${TARGET_SERVER}" in
      A) audit_server "$SERVER_A" "Server A" ;;
      B) audit_server "$SERVER_B" "Server B" ;;
      C) audit_server "$SERVER_C" "Server C" ;;
      *)
        audit_server "$SERVER_A" "Server A"
        audit_server "$SERVER_B" "Server B"
        audit_server "$SERVER_C" "Server C"
        ;;
    esac
  fi
  ...
  if [[ "$has_issues" == true ]] || [[ "$EMAIL_ON_SUCCESS" == true ]]; then
    ...
    local email_subject="[ClawNet Testnet] Security Audit ${level} — $(hostname) — ${TIMESTAMP}"
    local email_body
    email_body=$(printf "ClawNet Testnet Security Audit Report\n\n"
    printf "Status:   %s\n" "$level"
    printf "Passed:   %d\n" "$PASS"
    printf "Warnings: %d\n" "$WARN"
    printf "Failed:   %d\n" "$FAIL"
