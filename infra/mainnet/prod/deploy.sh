...
DEPLOY_STASH_SCOPE="mainnet-deploy"
...
install_caddy_and_webapps() {
  ...
  scp_to "$SCRIPT_DIR/../../mainnet/Caddyfile" "$host" "/etc/caddy/Caddyfile"
  ...
  # 4. Build console (standalone mode — served at root of console.clawnet.io)
...
systemctl restart clawnet-docs"
  ...
  run_remote "$host" "mkdir -p /var/www/clawnet"
...
# ══════════════════════════════════════════════════════════════════
# Phase 14: Deploy Caddy + all web apps on Node 1
# ══════════════════════════════════════════════════════════════════
echo ">>> Phase 14: Deploying Caddy + all web apps on Node 1..."
install_caddy_and_webapps "$SERVER_1"
echo "  Caddy + all web apps deployed."
echo ""
