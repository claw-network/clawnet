...
ClawNet mainnet 提供两个监控脚本：**health-check.sh**（高频巡检）和 **daily-monitor.sh**（每日综合报告）。
...
```
                  ┌─────────────────────────────────────┐
                  │  Geth Validator Cluster (5-Node)     │
                  │                                     │
  health-check.sh │  Node 1 (IP TBD)                    │
  daily-monitor.sh│    ├─ Geth RPC (0.0.0.0:8545) ◄────┤── 外部可达
       ▼          │    ├─ Caddy (api.clawnet.io)        │
  ┌────────┐      │    └─ clawnetd (:9528)              │
  │ 本地   │      │                                     │
  │ 或     │─────►│  Node 2 (IP TBD)                    │
...
Node 2-5 的 RPC 绑定在 127.0.0.1，外部不可达。通过 Node 1 的 `net_peerCount ≥ 4` 间接验证 Node 2-5 在线。
...
| 3 | Peer 数量 | `net_peerCount` | ≥ 4（5 节点集群） |
...
```bash
# 远程模式（本地或 CI 运行，通过 Node 1 的公开 RPC 检查）
./health-check.sh
...
```crontab
# 每 5 分钟巡检，输出到 syslog
*/5 * * * * /opt/clawnet/infra/mainnet/health-check.sh 2>&1 | logger -t clawnet-health
```
...
- 需要配置 `NODE_1_IP` 或 `GETH_RPC_1`（mainnet IP 部署后设置）
...
**用途**：每日综合稳定性报告，包含 4 大检查模块 + JSON 报告输出。用于上线后观察窗口（7 天）。

### 检查模块

| # | 模块 | 说明 | 运行位置 |
|---|------|------|----------|
| 1 | **Geth Chain Health** | 区块高度、新鲜度、peer 数量(≥4)、集群连通性 | 本地 + 服务器 |
...
> **注意**：此模块需要 `indexer.sqlite`（仅存在于运行 clawnetd 的服务器上）+ hardhat 连接 RPC（使用 `--network clawnetMainnet`）。本地运行时会自动跳过并显示 `ℹ` 提示。  
> 从 2026-02-28 起：若检测到服务器 `clawnetd-data/config.yaml` 启用了 `chain:` 但缺少 `indexer.sqlite`，监控会直接标记 `FAIL`（不再静默跳过）。

### Scenario 智能检测

脚本会自动读取 `scenarios/.env` 中的节点 URL：

- **多个不同 URL** → 正常运行 Scenario 01
- **所有 URL 相同** → 自动跳过并提示「单节点，等多节点部署后自动启用」
- **URL 未配置** → 自动跳过
...
```crontab
# 每天 UTC 06:00 运行
0 6 * * * cd /opt/clawnet && bash infra/mainnet/daily-monitor.sh 2>&1 | tee -a /opt/clawnet/logs/monitor.log
```

### 输出报告

每次运行会在 `infra/mainnet/reports/` 下生成 JSON 报告：

```
reports/
├── 2026-03-15.json            # 每日综合报告
├── reconcile-2026-03-15.json  # Reconciliation 详细报告（仅服务器运行时）
└── ...
```
...
```json
{
  ...
  "observationTotal": 7,
  "checks": {
    "geth": { "blockHeight": 1000, "clusterPeers": 4, "expectedPeers": 4 },
    "nodeApi": { "node1": "ok" },
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `NODE_1_IP` | （空，部署时设置） | Node 1 服务器 IP |
| `GETH_RPC_1` | 由 `NODE_1_IP` 推导 | Node 1 的 Geth RPC |
| `NODE_1_URL` | `https://api.clawnet.io` | ClawNet Node API（Caddy 反代） |
| `OBSERVATION_START` | `2026-03-15` | 观察窗口起始日期 |
...
| Chain ID | 7625 | 7626 |
| 期望 peer 数 | ≥ 2 | ≥ 4 |
| Reconciliation 网络 | `--network clawnetTestnet` | `--network clawnetMainnet` |
| API 域名 | `api.clawnetd.com` | `api.clawnet.io` |
| RPC 域名 | `rpc.clawnetd.com` | `rpc.clawnet.io` |
| 观察窗口起始 | 2026-02-26 | 2026-03-15（预计） |
