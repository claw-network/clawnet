...
> 3 台服务器部署 ClawNet Chain（Hyperledger Besu QBFT）的完整操作手册。
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│          3 台服务器 · ClawNet Chain (Besu QBFT) 最小生产级部署        │
│                                                                              │
│  ┌──────────────────────┐  ┌──────────────────────┐                         │
│  │  Server B            │  │  Server C            │                         │
│  │  85.239.236.49       │  │  85.239.235.67       │                         │
│  │                      │  │                      │                         │
│  │  Besu Validator #2   │  │  Besu Validator #3   │                         │
...
│  │    rpc.clawnetd.com:443   → Besu JSON-RPC         │                      │
│  │    :30303                 → Besu P2P              │                      │
│  └────────────────────────────────────────────────────┘                      │
│                                                                              │
│  QBFT 共识：3 Validators · 零手续费 (zeroBaseFee + min-gas-price=0)         │
...
| 组件       | 选择                       | 原因                                                          |
| ---------- | -------------------------- | ------------------------------------------------------------- |
| EVM 客户端 | **Hyperledger Besu**       | 原生支持 QBFT BFT 共识，活跃维护，支持 `--min-gas-price=0` 零手续费 |
| 共识协议   | **QBFT (BFT)**             | Istanbul BFT 改进版，适合许可链，3f+1 容错（3 验证者容忍 0 宕机） |
...
Agent/SDK/Browser
  ...
  │
  ├──(HTTPS)──→ rpc.clawnetd.com:443 ──→ Caddy ──→ Besu :8545 (JSON-RPC)
  │
  └──(TCP)────→ :30303 ──→ Besu P2P (devp2p)
...
| 443   | TCP     | HTTPS (Caddy)             | ✅         | ❌           |
| 8545  | TCP     | Besu JSON-RPC             | Caddy 反代 | 仅 127.0.0.1 |
| 9527  | TCP     | clawnetd P2P (预留)       | ✅         | ✅           |
| 30303 | TCP+UDP | Besu P2P (devp2p)         | ✅         | ✅           |
...
- `genesis.json` 存在性和 QBFT 兼容性校验（必须包含 `qbft` 和 `zeroBaseFee`）
...
| --- | --- | --- | --- | --- |
| `VALIDATOR_1_ADDRESS` | EOA 地址 | 由 `VALIDATOR_1_PRIVATE_KEY` 推导 | `infra/testnet/prod/deploy.sh` Phase 2/9/10 | Genesis extradata 验证者 + `NODE_ADDRESSES`（bootstrap 分配） |
| `VALIDATOR_1_PRIVATE_KEY` | 私钥 | 本地生成/硬件导出 | `infra/testnet/prod/deploy.sh` Phase 2 | 写入 `/opt/clawnet/config/key`，Besu 用于 QBFT 签名 |
| `VALIDATOR_2_ADDRESS` | EOA 地址 | 由 `VALIDATOR_2_PRIVATE_KEY` 推导 | `infra/testnet/prod/deploy.sh` Phase 2/9/10 | 同上（验证者 #2） |
| `VALIDATOR_2_PRIVATE_KEY` | 私钥 | 本地生成/硬件导出 | `infra/testnet/prod/deploy.sh` Phase 2 | 写入 key 文件（验证者 #2） |
| `VALIDATOR_3_ADDRESS` | EOA 地址 | 由 `VALIDATOR_3_PRIVATE_KEY` 推导 | `infra/testnet/prod/deploy.sh` Phase 2/9/10 | 同上（验证者 #3） |
| `VALIDATOR_3_PRIVATE_KEY` | 私钥 | 本地生成/硬件导出 | `infra/testnet/prod/deploy.sh` Phase 2 | 写入 key 文件（验证者 #3） |
...
| `VALIDATOR_PASSWORD` | ~~不再使用~~ | — | — | Besu 直接读取 key 文件，无需 keystore 密码 |
...
| qbft.epochlength         | 30000      | epoch 间隔            |
| qbft.requesttimeoutseconds | 4       | 请求超时              |
...
extradata 是 QBFT 的核心字段，RLP 编码了初始验证者集合：

```
格式: 0x + 32字节vanity(64个0) + RLP([排序的验证者地址列表, votes=[], round=0, seals=[]])
```

使用项目提供的 extradata 生成器：
...
### 3.4 创建 genesis.json 和 key 文件

```bash
# 将完成的 genesis.json 放到 /opt/clawnet/config/genesis.json
# 可以 scp 上传或直接 cat 写入（参考 Step 2.4）

# 写入验证者私钥文件（Besu 直接读取 hex key，无需 keystore/password）
echo "<VALIDATOR_1_PRIVATE_KEY_WITHOUT_0x>" > /opt/clawnet/config/key
chmod 600 /opt/clawnet/config/key
```

### 3.5 启动 Besu

Besu 无需 `init` 或 `account import` 步骤。Besu 在首次启动时自动从 genesis.json 初始化链数据，
并直接从 `--node-private-key-file` 读取验证者私钥。

### 3.6 部署 docker-compose 文件
...
# 查看日志（确认出块）
docker logs -f clawnet-besu --tail 20
...
ufw allow 30303/tcp    # Besu P2P
ufw allow 30303/udp    # Besu P2P discovery
ufw enable
```
...
# 写入验证者私钥（Besu 直接读取 hex key，去掉 0x 前缀）
echo "<VALIDATOR_2_PRIVATE_KEY_WITHOUT_0x>" > /opt/clawnet/config/key
chmod 600 /opt/clawnet/config/key
```

### 4.3 以 peer 模式启动

> Besu 无需 `init` 或 `account import`。首次启动时自动从 genesis.json 初始化。

> Besu QBFT 无需「先同步再出块」策略。QBFT 共识协议会自动处理验证者加入和同步。
> 只需用 `docker-compose.peer.yml` 直接启动即可。

```bash
# 使用 peer 模式的 compose（含 --bootnodes 指向 Server A）
...
| 验证者密钥（`/config/key`）      | Validator 2         | **Validator 3**                           |
...
> 我们的 QBFT 链不支持 Shanghai/Cancun EVM 特性。
...
- [ ] Server A Besu 出块中（区块号增长）
- [ ] Server B Besu 同步中（区块号一致）
- [ ] Server C Besu 同步中（区块号一致）
- [ ] 每台 Besu peer 数量 = 2
...
docker logs clawnet-besu --tail 20
```
...
> QBFT 共识需要 2f+1 验证者在线（3 验证者中需要 2 个在线）。
...
# 2. 删除损坏的链数据（key 文件在 /config 中，不受影响）
rm -rf /opt/clawnet/chain-data/*

# 3. 重新启动（Besu 自动从 genesis.json 重新初始化）
docker compose -f docker-compose.chain.yml up -d
# Besu 会自动从其他节点同步链数据
```
...
1. 购买新 VPS
2. 使用相同的验证者私钥（写入 `/opt/clawnet/config/key`）和 genesis.json 重新部署
3. 用 `docker-compose.peer.yml` 启动，QBFT 会自动同步
...
### 11.1 为什么选择 Besu 而非 Geth/Reth

- Reth 是纯 PoS 客户端，不支持 QBFT/Clique。
- Geth v1.14+ 移除了 Clique PoA，v1.13.15 是最后支持版本（EOL，无安全更新）。
- Besu 原生支持 QBFT BFT 共识，活跃维护，且内置 `--min-gas-price=0` 和 `zeroBaseFee`。

### 11.2 QBFT 与 Clique 的关键差异

| 特性 | Clique (Geth) | QBFT (Besu) |
|------|---------------|-------------|
| 初始化 | 需要 `geth init` | 首次启动自动初始化 |
| 密钥管理 | keystore + password | 直接读取 hex key 文件 |
| 启动策略 | sync-first-then-mine | 直接启动，自动加入共识 |
| 最终性 | 概率性 | BFT 确定性最终性 |
| 零手续费 | 需要 patch 源码 | 原生 `zeroBaseFee: true` |

### 11.3 Genesis 只支持到 London EVM

Genesis 中不要包含 `shanghaiTime` 等 time-based fork 字段。
链 EVM 版本为 London（支持 EIP-1559），合约编译也必须用 `london`。

### 11.4 QBFT 需要 2f+1 验证者

3 个验证者的 QBFT 网络容错 f=0（即 3 个都必须在线才能出块）。
如需容忍 1 台宕机，需要 4 个验证者（4 = 3f+1, f=1）。

### 11.5 Besu 节点 key 与 enode

Besu 将节点身份 key 保存在 `data-path/key`（自动生成）。
但当使用 `--node-private-key-file=/config/key` 时，该文件同时用于 P2P 身份和 QBFT 签名。
如果需要迁移节点，保留 key 文件即可保持 enode URL 不变。
...
### 11.9 Besu 容器的 entrypoint

`hyperledger/besu` 镜像的 ENTRYPOINT 是 `besu`。
compose 文件中用 `entrypoint:` 列出参数。Besu 无需 `init`，首次启动自动初始化。
...
| 阶段           | 操作                                       | 影响                      |
| -------------- | ------------------------------------------ | ------------------------- |
| 用户增长       | 加第 4 台 Besu Full Node（只读 RPC）       | 分担 Server A 的 RPC 负载 |
...
已完成从 Geth Clique PoA 到 Hyperledger Besu QBFT 的迁移。

| 维度     | 当前 QBFT（Besu）         | 目标 PoS             |
| -------- | ------------------------- | -------------------- |
| 适用阶段 | testnet/早期主网          | 长期开放网络         |
| 复杂度   | 中                        | 高                   |
| 运维成本 | 中                        | 高                   |
| 最终性   | BFT 最终性（强）          | 强最终性（取决实现） |
| 推荐动作 | 维持并加固                | 仅在条件成熟后启动   |
...
1. `Now -> 1-2 个月`：继续 PoA，锁定 `Besu (latest)` + London Genesis，优先稳定性与运维自动化。
...
```
infra/testnet/
├── genesis.json                   ← 创世区块（3 个 validator 地址）
...
```
infra/mainnet/
├── README.md                      ← Mainnet 部署指南（5 节点拓扑）
├── genesis-template.json          ← 创世区块模板（5 个 validator 占位符）
├── docker-compose.yml             ← Node 1 — 主验证者 compose（挖矿模式）
├── docker-compose.peer.yml        ← Node 2-5 — 对等验证者 compose（挖矿模式）
├── docker-compose.sync.yml        ← 同步模式 compose（不挖矿，用于新节点加入）
├── Caddyfile                      ← Node 1 Caddy 配置（clawnet.io 域名）
├── .env.example                   ← 环境变量模板
├── .gitignore                     ← 忽略 secrets / chain-data 等
├── health-check.sh                ← 健康检查脚本（5 节点）
├── setup-server.sh                ← 新服务器初始化脚本
├── prod/
│   ├── deploy.sh                  ← 一键部署脚本（5 节点 preflight + 自动化）
│   ├── secrets.env.example        ← 部署密钥模板
│   └── .gitignore                 ← 忽略 secrets / keys / contracts.json
└── multisig-soft-wallet/
    ├── README.md                  ← Mainnet 多签说明（复用 testnet 脚本）
    ├── create-safe-addresses.sh   ← Mainnet Safe 创建脚本（clawnetMainnet 网络）
...
_最后更新: 2025-02-26_
_适用版本: ClawNet Chain v0.1_
_Geth: v1.13.15 · QBFT · Testnet Chain ID: 7625 · Mainnet Chain ID: 7626_
