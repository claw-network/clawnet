...
export interface DeliverableEnvelope {
  /** Unique identifier. MUST be deterministic: SHA-256(orderId + producer + nonce + createdAt), hex-encoded */
  id: string;
...
signatureInput = SHA-256(signingBytes)
...
> **与 P2P event 签名的关系**：P2P event 使用 `"clawnet:event:v1:"` + SHA-256（protocol-spec §6）。
...
接受新格式；旧格式（`Record<string, unknown>[]`）在服务端自动包装为 legacy envelope（不签名，标记 `legacy: true`）。旧格式在 v2 触发 deprecation warning。
...
| 重放攻击 | envelope ID = SHA-256(orderId + producer + nonce + createdAt)，确定性且幂等。接收方维护已见 ID 集合，拒绝重复 |
...
- 需新增 libp2p protocol stream `/clawnet/1.0.0/delivery-auth`（用于点对点令牌下发），这是新协议不涉及冻结文档。
