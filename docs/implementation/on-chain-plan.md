...
│     → 批量提交，减少单笔交易上链频率                                 │
│     → 独立 EVM 链，Gas 收入 100% 归 ClawNet，零外部支付             │
│                                                                      │
│  5. 双轨运行期                                                       │
│     → 链下系统与链上系统并行                                         │
...
│                                                                      │
│  ┌─────────────┬──────────┬─────────┬─────────┬──────────────────┐  │
│  │             │ 独立 EVM │ 自建 L3 │ Base L2 │Cosmos SDK       │  │
│  ├─────────────┼──────────┼─────────┼─────────┼──────────────────┤  │
│  │ Gas 成本    │  自定    │  极低   │  低     │  自定           │  │
│  │ Gas 收入    │ 100%归己 │ 100%归己│  归Base │  100%归己       │  │
│  │ 第三方依赖  │  零      │  Base+L1│  Base+L1│  零             │  │
│  │ ETH 支付    │  零      │  需要   │  需要   │  零             │  │
│  │ 自定义Gas币 │  ✅      │  ✅     │  ❌     │  ✅             │  │
│  │ EVM 兼容    │  ✅      │  ✅     │  ✅     │  需额外适配     │  │
│  │ 自定义预编译│  ✅完全  │  部分   │  ❌     │  需 CosmWasm    │  │
│  │ 共识自主    │  完全    │  部分   │  无     │  完全           │  │
│  │ Solidity 复用│ ✅      │  ✅     │  ✅     │  ❌             │  │
│  │ 工具链成熟度│  最高    │  高     │  高     │  中             │  │
│  └─────────────┴──────────┴─────────┴─────────┴──────────────────┘  │
│                                                                      │
│  推荐：独立 EVM 链（Fork Reth / Geth）                              │
│  核心原则：零第三方依赖，零 ETH 支付，完全自主                      │
│  原因：                                                              │
│    1. Gas 收入 100% 归 ClawNet 协议（无需向任何第三方支付）         │
│    2. Token 作为原生 Gas 代币（用户无需持有 ETH）                   │
│    3. 完全 EVM 兼容，复用以太坊 Solidity/Hardhat 工具链             │
│    4. 完全自主的共识机制（PoA → PoS 渐进过渡）                      │
│    5. 可自由添加自定义预编译（Ed25519 等）                           │
│    6. 不依赖任何外部结算层或数据可用性层                             │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
┌────────────────────────────────────────────────────────────┐
│                ClawNet Chain（独立 EVM 链）                 │
│                                                            │
│  基础：Fork Reth（Rust 实现，高性能）或 Geth（Go 实现）   │
│  共识：初期 PoA（Clique）→ 渐进过渡到 PoS（基于质押）    │
│  Gas 代币：Token（原生代币，非 ERC-20 wrapper）            │
│  出块时间：~2 秒（PoA），~6 秒（PoS 阶段可调）           │
│  数据可用性：自有节点全量存储（无外部 DA 依赖）            │
│                                                            │
│  收入模型：                                                │
│  ├── 用户每笔交易支付 Gas（以 Token 计价）                │
│  ├── PoA 阶段：Gas 收入 → 协议国库（100%）               │
│  ├── PoS 阶段：Gas 收入 → 验证者 + 协议国库（比例可调） │
│  └── 零外部成本，Gas 收入即净收入                          │
│                                                            │
│  安全模型：                                                │
│  ├── PoA 阶段：ClawNet 团队运营授权节点，BFT 容错         │
│  ├── PoS 阶段：节点质押 Token + 惩罚机制保障安全          │
│  └── 不依赖外部链安全性，完全自主保障                      │
│                                                            │
│  Ed25519 支持：                                            │
│  ├── 自定义预编译合约，原生支持 Ed25519 签名验证          │
│  └── 无需 EIP 审批流程，完全自主添加                       │
│                                                            │
└────────────────────────────────────────────────────────────┘
```
...
Phase W3: 测试网部署                                [2 周]
├── 部署至 ClawNet Chain 测试网（本地 devnet）
├── 运行 clawnet 集成测试场景
└── 压力测试（并发转账、托管创建/释放）
...
2. **Ed25519 签名验证**：独立链可自由添加自定义预编译原生支持 Ed25519，
   无需 EIP 审批流程；备选方案：纯 Solidity 实现或链下验证 + 链上提交。
...
| 操作 | 频率 | 预估 Gas (ClawNet Chain) | 月成本估算 |
|------|------|-------------------|-----------|
| 批量信誉锚定 (100 个 Agent) | 每 24h | ~200,000 gas | 极低（Gas 以 Token 计价，收入归协议，零外部成本） |
...
2026 Q2                    2026 Q3                    2026 Q4                    2027 Q1
  ...
  │ │• 链节点软件搭建 │    │ │• ClawStaking     │    │ │• 外部审计        │    │ │• 切换 source of   │
  │ │  (Reth/Geth Fork)│    │ │• 测试网部署      │    │ │• Bug Bounty 启动 │    │ │  truth            │
  │ │• Ed25519 预编译  │    │ │• SDK 适配        │    │ │• SDK 适配        │    │ │• 链下系统退役     │
  │ └──────────────────┘    │ └──────────────────┘    │ └──────────────────┘    │ └──────────────────┘
  │                          │                          │                          │
  │ 4 周                     │ 10 周                    │ 12 周                    │ 8 周
```
...
Phase 0: 基础设施
  ...
  CI/CD + 自动化测试                      ████
  OpenZeppelin 集成                       ████
  链节点 Reth/Geth Fork + Ed25519 预编译       ████
...
| Gas 成本超预期 | 用户体验差 | 低 | 独立链完全可调 Gas 参数 + 批量操作 + 链下优先策略 |
| Ed25519 兼容问题 | 身份系统无法迁移 | 极低 | 独立链可自由添加自定义预编译，无需外部审批 |
| 迁移数据不一致 | 余额错误 | 中 | 双轨运行期 + 自动对账 + 回滚机制 |
| 外部审计延期 | 上线延迟 | 中 | 提前预约 + 并行多家审计 |
| 早期验证者不足 | 共识安全风险 | 中 | PoA 阶段由团队运营可信节点 + 逐步开放质押 |
| 链节点软件缺陷 | 链停机/分叉 | 中 | 基于成熟 Reth/Geth 代码 + 充分测试 + 灰度上线 |
...
infra/                              # 新增 — 链节点基础设施
  chain-devnet/
    ├── docker-compose.yml          # 本地 devnet (Reth/Geth PoA)
    ├── genesis.json                # 创世配置（PoA 授权节点、Token 预分配等）
    ├── chain.toml                  # Reth 链参数配置（出块时间、Gas 上限等）
    ├── node-key/                   # 授权节点密钥（仅开发用）
    └── precompiles/                # Ed25519 等自定义预编译配置
  chain-testnet/
    └── ...                         # 测试网部署配置
  chain-mainnet/
    └── ...                         # 主网部署配置
...
| # | 决策 | 选项 | 结论 | 原因 |
|---|------|------|------|------|
| 1 | 主链选择 | Ethereum L1 / Base L2 / 自建 L3 / Cosmos SDK / 独立 EVM | 独立 EVM 链 (Fork Reth/Geth) | 零第三方依赖 + 零 ETH 支付 + Gas 收入 100% 归协议 + Token 原生 Gas + 完全自主共识 + 自定义预编译 |
