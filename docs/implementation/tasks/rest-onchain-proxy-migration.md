...
| `services/staking-service.ts`    | `ClawStaking.sol`                  | _(本次迁移不在范围内——暂无 REST 端点，见 [§11](#11-不在范围内))_ |
...
- [x] **Node 层**: 在 `packages/node/src/api/server.ts` 中，identity 写操作路由内部调用 `IdentityService`（ethers.js → `ClawIdentity.sol`）
- [x] **Capability 决策**: Capability 凭证保持链下 P2P 存储，不迁移到链上（体量大、更新频繁，不适合链上存储）
- [x] **Indexer**: 监听 `DIDRegistered`, `KeyRotated`, `DIDRevoked` 事件，维护本地 DID 缓存（已在 EventIndexer 中实现）
...
- [x] **Node `WalletService`**: 实现全部钱包写操作（transfer, escrow CRUD），包括 `fundEscrow()` 和 `expireEscrow()`
- [x] **Node 层**: balance 查询增加 DID → address 解析逻辑 (`did` 参数通过 `ClawIdentity.getController()` 转换为 `address`)
- [x] **Indexer**: 监听 `Transfer`, `EscrowCreated`, `EscrowReleased`, `EscrowRefunded`, `EscrowExpired` 事件
...
| REST 端点                                | 迁移后实现                                             |
| ---------------------------------------- | ------------------------------------------------------ |
| `POST /api/v1/reputations/{did}/reviews` | `ClawReputation.recordReview()` + `anchorReputation()` |
...
- [x] **Node `ReputationService`**: record 路由调用 `ClawReputation.recordReview()` + `anchorReputation()`
- [x] **Indexer**: 监听 `ReputationAnchored`, `ReviewRecorded` 事件，维护可搜索的 review 列表
- [x] **Indexer 查询**: 支持按 `subjectDIDHash` 分页查询 reviews
...
- [x] **评估 `settlement`**: `ClawContracts.sol` 无独立 settle 方法，映射为 `terminateContract()`（退还剩余资金）
- [x] **Indexer**: 监听合约生命周期事件，建立可筛选的合约列表（新增 Terminated/Cancelled/DisputeResolved 事件处理）
- [x] **Indexer 查询**: 支持 `status`, `party` 筛选 + `limit/offset` 分页
...
| **P0.1** 设计 Node 服务层抽象（已完成 ✅） | `packages/node/src/services/`                | 创建 `ContractProvider` 抽象类/接口，封装 ethers Provider + Signer 管理                                                                                                                                |
| **P0.2** 配置管理（已完成 ✅）             | `packages/node/src/services/chain-config.ts` | 新增链上配置项：RPC URL、合约地址表、Signer 密钥路径（Zod schema 验证）                                                                                                                                |
| **P0.3** Event Indexer 核心（已完成 ✅）   | `packages/node/src/indexer/`                 | 实现基于 ethers.js `eth_getLogs` 轮询的事件监听 + SQLite 存储                                                                                                                                          |
| **P0.4** Indexer 通用查询（已完成 ✅）     | `packages/node/src/indexer/query.ts`         | 分页、筛选的通用查询层                                                                                                                                                                                 |
...
| 任务                                            | 说明                                                                                                     |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| **P1.1** Node `WalletService` 实现（已完成 ✅） | 实现全部钱包写操作（transfer, escrow CRUD），直接通过 ethers.js 调用 `ClawToken.sol` / `ClawEscrow.sol`  |
| **P1.2** Node wallet 路由改造（已完成 ✅）      | `POST /api/wallet/transfer` 内部调用 `WalletService.transfer()`                                          |
| **P1.3** Node escrow 路由改造（已完成 ✅）      | 所有 escrow POST 路由调用 `WalletService.*`                                                              |
| **P1.4** Balance 读操作改造（已完成 ✅）        | `GET /api/wallet/balance` 调用 `ClawToken.balanceOf()`；DID 参数通过 `ClawIdentity.getController()` 解析 |
| **P1.5** History indexer（已完成 ✅）           | 实现 `Transfer` event 监听 → `wallet_transfers` 表                                                       |
| **P1.6** History 查询路由（已完成 ✅）          | `GET /api/wallet/history` 从 indexer 查询                                                                |
...
| 任务                                           | 说明                                                                                |
| ---------------------------------------------- | ----------------------------------------------------------------------------------- |
| **P2.1** Node identity 写路由改造（已完成 ✅） | 注册/密钥轮换/吊销操作调用 Node `IdentityService`（ethers.js → `ClawIdentity.sol`） |
| **P2.2** Identity 读操作改造（已完成 ✅）      | `resolve()` 调链上 `getActiveKey()` + `getController()`                             |
| **P2.3** Capability 保留（已完成 ✅）          | Capability CRUD 路由保持现有 P2P/链下逻辑不变                                       |
| **P2.4** DID indexer（已完成 ✅）              | 监听 `DIDRegistered`, `KeyRotated`, `DIDRevoked` → 本地 DID 缓存                    |
| **P2.5** 集成测试                              |                                                                                     |
...
| 任务                               | 说明                                                                                           |
| ---------------------------------- | ---------------------------------------------------------------------------------------------- |
| **P4.1** 评估 `settlement`         | ✅ `ClawContracts.sol` 无独立 settle，映射为 `terminateContract()`（退还剩余资金）             |
...
| **P4.4** Contracts indexer         | ✅ 监听合约生命周期事件 → `service_contracts` 表（新增 Terminated/Cancelled/DisputeResolved）  |
...
| _(Node 新增)_ `test/services/*.test.ts`    | **新增** — Node 服务层单元测试（如 `wallet-service.test.ts`）           |
| _(Node 新增)_ `test/integration/*.test.ts` | **新增** — REST → 链上全链路集成测试（如 `wallet-integration.test.ts`） |
...
- [x] 场景测试通过（104 passed, 0 failed — 旧 clawnet/scenarios/ 已迁移至 infra/testnet/scenarios/）
...
| _(新增)_            | —                                                      | 新增 §7 "Event Indexer 安全"：Indexer 完整性校验、reorg 处理、数据一致性审计                                         |
