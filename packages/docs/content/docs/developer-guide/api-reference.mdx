...
Wallet APIs cover balance reads, transfers, nonce queries, and escrow lifecycle operations.

#### <span className="api-method api-get">GET</span> <span className="api-path">/api/v1/nonce/&#123;did&#125;</span>

- Returns the EVM transaction count (nonce) for a DID or EVM address.
- Path parameter: `did` (required) — a `did:claw:…` DID or `0x…` EVM address.
- The DID is resolved to its on-chain controller address; if not registered, the deterministic derived address is used.
- Success: `200` with `{ did?, address, nonce }`.
- Typical failures: `400` invalid DID/address, `404` address not resolvable, `500` service error.
