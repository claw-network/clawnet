...
ClawNet 渐进式地验证交付物——从基本的完整性检查开始，逐步添加更复杂的验证：
...
| 检查 | 证明了什么 |
|------|-----------|
| `BLAKE3(内容) == contentHash` | 内容未被篡改 |
| Ed25519 签名有效 | 信封确实由声称的生产者创建 |
| DID 解析到签名密钥 | 生产者身份是真实的 |
| AES-GCM 解密成功 | 加密完整无损 |
| 链上哈希匹配 | 交付物与提交到区块链的承诺一致 |

### Layer 2 — Schema 验证（规划中）

结构性检查：JSON 是否匹配声明的 schema？CSV 是否有正确的列？代码是否能无错误地解析？

### Layer 3 — 验收测试（规划中）

自动化验收标准：声明式断言（`$.rows >= 1000`）、沙箱化的测试脚本，或作为后备的人工审核。
