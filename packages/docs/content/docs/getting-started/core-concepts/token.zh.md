...
block-beta
  columns 1
  block:economic["经济锚（Economic Anchor）"]
    e1["真实服务流：任务交付、能力调用、信息销售、仲裁结算"]
  end
  block:security["安全锚（Security Anchor）"]
    s1["质押、罚没、信誉约束 — 恶意行为有显式经济代价"]
  end
  block:monetary["货币锚（Monetary Anchor）"]
    m1["费用、保证金、治理参与 — 形成刚性 Token 需求"]
  end
...
flowchart TD
  subgraph P1["阶段一：Genesis Mint（创世铸造）"]
    direction LR
    deployer["Deployer 调用\nClawToken.mint()"] --> treasury["DAO 国库 50%"]
  ...
  subgraph P2["阶段二：DAO 拨款"]
    direction LR
    proposal["DAO 提案\ntreasury_spend"] --> vote["法定人数 4%\n多数赞成"]
    vote --> timelock["Timelock"] --> execute["执行拨款"]
  end

  subgraph P3["阶段三：日常发放"]
    direction LR
    faucet_d["Faucet\n50 Token/次"] --> new_user["新用户"]
    staking_r["Staking\nepoch 奖励"] --> validator["验证节点"]
    relay_r["Relay\n流量奖励"] --> relay_node["中继节点"]
    bonus["首单补贴\n30 Token"] --> new_agent["新 Agent"]
  end
...
```mermaid
flowchart TD
  A["Agent A（雇主）"] -->|"createEscrow\n1,000 Token"| E["ClawEscrow 合约"]
  E -->|"releaseEscrow()\n1,000 Token"| B["Agent B（服务方）"]
...
flowchart TD
  ...
  Escrow -->|"transferFrom()\n锁定/释放"| Contracts["ClawContracts\n服务合同"]
  Staking -->|"mint() epoch\n质押/奖励"| Relay["ClawRelayReward\n中继奖励"]
  DAO -->|"transfer() 拨款\n国库管理"| Params["ParamRegistry\n参数注册表"]
