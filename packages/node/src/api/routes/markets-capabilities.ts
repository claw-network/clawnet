...
import {
  CapabilityPublishSchema, CapabilityLeaseSchema,
  CapabilityLeaseActionSchema, CapabilityInvokeSchema,
...
export function marketsCapabilityRoutes(ctx: RuntimeContext): Router {
  ...
  r.post('/', async (_req, res, route) => {
    ...
    const body = v.data;
    const listingId = body.listingId ?? `cap-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    try {
      const envelope = await createCapabilityListingPublishEnvelope({
        ...
        marketData: {
          capabilityType: body.capabilityType, capability: body.capability,
          performance: body.performance, quota: body.quota,
          access: body.access, sla: body.sla,
        } as never,
    ...
    } catch { internalError(res, 'Capability listing publish failed'); }
  });
    ...
    if (ctx.searchMarkets) {
      try {
        const result = ctx.searchMarkets({
          markets: ['capability'], page, pageSize: perPage,
  ...
  r.get('/:id', async (_req, res, route) => {
    ...
    if (ctx.marketStore) {
      try {
        ...
        if (listing) { ok(res, listing, { self: `/api/v1/markets/capabilities/${id}` }); return; }
      } catch { /* fallthrough */ }
    }
  ...
  r.delete('/:id', async (_req, res, route) => {
    ...
    } catch { internalError(res, 'Capability listing remove failed'); }
  });
  ...
  r.post('/:id/leases', async (_req, res, route) => {
    ...
    if (!privateKey) { badRequest(res, 'Key unavailable', route.url.pathname); return; }
    try {
      const leaseId = body.leaseId ?? `lease-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const envelope = await createMarketCapabilityLeaseStartEnvelope({
        issuer: body.did, privateKey, listingId: id, leaseId,
        plan: body.plan, credentials: body.credentials,
        metadata: body.metadata, expiresAt: body.expiresAt,
        resourcePrev: body.resourcePrev as never,
        ts: body.ts ?? Date.now(), nonce: body.nonce, prev: body.prev,
      });
      const hash = await ctx.publishEvent(envelope);
      created(res, { leaseId, txHash: hash, credentials: body.credentials },
        { self: `/api/v1/markets/capabilities/${id}/leases/${leaseId}` });
    } catch { internalError(res, 'Lease start failed'); }
  });
  ...
  r.post('/:id/leases/:leaseId/actions/invoke', async (_req, res, route) => {
    ...
    try {
      const envelope = await createMarketCapabilityInvokeEnvelope({
        issuer: body.did, privateKey, leaseId,
        resource: body.resource, units: body.units as never,
        latency: body.latency, success: body.success,
      ...
      ok(res, {
        leaseId, txHash: hash,
        usage: { resource: body.resource, units: body.units, latency: body.latency, success: body.success },
      }, { self: `/api/v1/markets/capabilities/${id}/leases/${leaseId}` });
    } catch { internalError(res, 'Capability invoke failed'); }
  });
  ...
  r.post('/:id/leases/:leaseId/actions/pause', async (_req, res, route) => {
    ...
    try {
      ...
      const hash = await ctx.publishEvent(envelope);
      ok(res, { leaseId, txHash: hash, action: 'pause' },
        { self: `/api/v1/markets/capabilities/${id}/leases/${leaseId}` });
    } catch { internalError(res, 'Lease pause failed'); }
  });
  ...
  r.post('/:id/leases/:leaseId/actions/resume', async (_req, res, route) => {
    ...
    try {
      ...
      const hash = await ctx.publishEvent(envelope);
      ok(res, { leaseId, txHash: hash, action: 'resume' },
        { self: `/api/v1/markets/capabilities/${id}/leases/${leaseId}` });
    } catch { internalError(res, 'Lease resume failed'); }
  });
  ...
  r.post('/:id/leases/:leaseId/actions/terminate', async (_req, res, route) => {
    ...
    try {
      ...
      const hash = await ctx.publishEvent(envelope);
      ok(res, { leaseId, txHash: hash, action: 'terminate' },
        { self: `/api/v1/markets/capabilities/${id}/leases/${leaseId}` });
    } catch { internalError(res, 'Lease terminate failed'); }
  });
