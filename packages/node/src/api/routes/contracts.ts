...
import {
  ...
  ContractSignSchema,
  ContractFundSchema,
  ContractCompleteSchema,
  ContractMilestoneSubmitSchema,
  ContractMilestoneReviewSchema,
  ContractDisputeSchema,
  ContractDisputeResolveSchema,
...
export function contractRoutes(ctx: RuntimeContext): Router {
  ...
  r.post('/', async (_req, res, route) => {
    ...
    const body = v.data;
    const contractId =
      body.contractId ?? `contract-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    if (ctx.contractsService) {
      try {
        ...
        const result = await ctx.contractsService.createContract({
          ...
          deadline:
            ((body.timeline as Record<string, unknown> | undefined)?.deadline as number) ?? 0,
      ...
      try {
        ...
        if (party)
          filtered = filtered.filter(
            (c) => c.parties.client.did === party || c.parties.provider.did === party,
          );
    ...
    if (ctx.contractsService) {
      try {
        const result = await ctx.contractsService.disputeContract(
          id,
          body.evidence ? JSON.stringify(body.evidence) : (body.reason ?? ''),
        );
        ok(res, result, { self: `/api/v1/contracts/${id}` });
        return;
      } catch (err) {
    ...
    if (ctx.contractsService) {
      try {
        ...
        const result = await ctx.contractsService.resolveDispute(
          id,
          resolutionMap[body.resolution] ?? 0,
        );
        ok(res, result, { self: `/api/v1/contracts/${id}` });
        return;
      } catch (err) {
