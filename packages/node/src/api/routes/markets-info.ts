...
import {
  InfoPublishSchema, InfoPurchaseSchema, InfoDeliverSchema,
  InfoConfirmSchema, InfoReviewSchema, InfoSubscriptionSchema,
...
export function marketsInfoRoutes(ctx: RuntimeContext): Router {
  ...
  r.post('/', async (_req, res, route) => {
    ...
    const body = v.data;
    const listingId = body.listingId ?? `info-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    try {
      const envelope = await createInfoListingPublishEnvelope({
        issuer: body.did, privateKey, listingId,
        title: body.title, description: body.description,
        category: body.category, tags: (body.tags ?? []) as string[],
        pricing: body.pricing as never,
        visibility: (body.visibility ?? 'public') as never,
        marketData: {
          infoType: body.infoType, content: body.content,
          accessMethod: body.accessMethod, license: body.license,
          quality: body.quality, usageRestrictions: body.usageRestrictions,
        } as never,
        restrictions: body.restrictions as never,
        metadata: body.metadata as Record<string, unknown> | undefined,
        expiresAt: body.expiresAt, status: body.status as never,
        ts: body.ts ?? Date.now(), nonce: body.nonce ?? 0, prev: body.prev,
      } as never);
      const hash = await ctx.publishEvent(envelope);
      created(res, { listingId, txHash: hash },
        { self: `/api/v1/markets/info/${listingId}` });
    } catch { internalError(res, 'Info listing publish failed'); }
  });
    ...
    if (ctx.marketStore) {
      try {
        const result = ctx.searchMarkets?.({
          markets: ['info'], page, pageSize: perPage,
  ...
  r.get('/:id', async (_req, res, route) => {
    ...
    if (ctx.marketStore) {
      try {
        ...
        if (listing) { ok(res, listing, { self: `/api/v1/markets/info/${id}` }); return; }
      } catch { /* fallthrough */ }
    }
  ...
  r.delete('/:id', async (_req, res, route) => {
    ...
    try {
      const envelope = await createMarketListingRemoveEnvelope({
        issuer: body.did, privateKey, listingId: id,
        resourcePrev: body.prev ?? '',
        ts: body.ts ?? Date.now(), nonce: body.nonce ?? 0, prev: body.prev,
      });
      const hash = await ctx.publishEvent(envelope);
      ok(res, { listingId: id, txHash: hash }, { self: `/api/v1/markets/info/${id}` });
    } catch { internalError(res, 'Info listing remove failed'); }
  });
  ...
  r.post('/:id/actions/purchase', async (_req, res, route) => {
    ...
    try {
      ...
      const totalAmount = body.unitPrice
        ? String(Number(body.unitPrice) * (body.quantity ?? 1)) : '0';

      const e2 = await createInfoEscrowCreateEnvelope({
        issuer: body.did, privateKey, escrowId,
        buyerDid: body.did, sellerDid: ((body as Record<string, unknown>).sellerDid as string) ?? '',
        amount: totalAmount,
        releaseRules: body.releaseRules ?? [{ type: 'delivery_confirmation' }],
        ts: ts + 1, nonce: nonce + 1, prev: h1,
      });
      const h2 = await ctx.publishEvent(e2);

      const e3 = await createInfoEscrowFundEnvelope({
        issuer: body.did, privateKey, escrowId,
        amount: totalAmount, resourcePrev: h2,
        ts: ts + 2, nonce: nonce + 2, prev: h2,
      });
      const h3 = await ctx.publishEvent(e3);

      const e4 = await createInfoOrderPaymentEscrowedEnvelope({
        issuer: body.did, privateKey, orderId,
        escrowId, resourcePrev: h3,
        ts: ts + 3, nonce: nonce + 3, prev: h3,
      });
      const h4 = await ctx.publishEvent(e4);

      created(res, {
        orderId, escrowId,
        orderHash: h1, escrowCreateHash: h2, escrowFundHash: h3, paymentHash: h4,
      }, { self: `/api/v1/markets/info/${id}` });
    } catch { internalError(res, 'Info purchase failed'); }
  });
  ...
  r.post('/:id/actions/deliver', async (_req, res, route) => {
    ...
    try {
      ...
      const hash = await ctx.publishEvent(envelope);
      ok(res, { deliveryId, orderUpdateHash: hash },
        { self: `/api/v1/markets/info/${id}` });
    } catch { internalError(res, 'Info delivery failed'); }
  });
  ...
  r.post('/:id/actions/confirm', async (_req, res, route) => {
    ...
    try {
      ...
      const h2 = await ctx.publishEvent(e2);
      ok(res, { escrowReleaseHash: h1, orderUpdateHash: h2 },
        { self: `/api/v1/markets/info/${id}` });
    } catch { internalError(res, 'Info confirm failed'); }
  });
  ...
  r.post('/:id/actions/review', async (_req, res, route) => {
    ...
    } catch { internalError(res, 'Info review failed'); }
  });
  ...
  r.post('/:id/subscriptions', async (_req, res, route) => {
    ...
    if (!privateKey) { badRequest(res, 'Key unavailable', route.url.pathname); return; }
    try {
      const subscriptionId = body.subscriptionId ?? `sub-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const envelope = await createMarketSubscriptionStartEnvelope({
        issuer: body.did, privateKey, listingId: id, subscriptionId,
        ts: body.ts ?? Date.now(), nonce: body.nonce ?? 0, prev: body.prev,
      });
      const hash = await ctx.publishEvent(envelope);
      created(res, { subscriptionId, txHash: hash },
        { self: `/api/v1/markets/info/${id}/subscriptions/${subscriptionId}` });
    } catch { internalError(res, 'Subscription start failed'); }
  });
  ...
  r.delete('/:id/subscriptions/:subId', async (_req, res, route) => {
    ...
    try {
      const envelope = await createMarketSubscriptionCancelEnvelope({
        issuer: body.did, privateKey, subscriptionId: subId,
        resourcePrev: body.prev ?? '',
        ts: body.ts ?? Date.now(), nonce: body.nonce ?? 0, prev: body.prev,
      });
      const hash = await ctx.publishEvent(envelope);
      ok(res, { subscriptionId: subId, txHash: hash },
        { self: `/api/v1/markets/info/${route.params.id}` });
    } catch { internalError(res, 'Subscription cancel failed'); }
  });
