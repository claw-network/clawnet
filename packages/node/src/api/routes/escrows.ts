...
import {
  ok, created, badRequest, notFound, internalError, paginated, parsePagination,
} from '../response.js';
import { validate } from '../schemas/common.js';
import {
  EscrowCreateSchema, EscrowActionSchema, EscrowExpireSchema,
} from '../schemas/wallet.js';
import type { RuntimeContext } from '../types.js';
import {
  resolveAddress, resolvePrivateKey, addressFromDid,
  buildEscrowView, parseBigInt, mapEscrowStatus,
} from '../types.js';
...
export function escrowRoutes(ctx: RuntimeContext): Router {
  ...
  r.post('/', async (_req, res, route) => {
    ...
    if (!beneficiary) { badRequest(res, 'Invalid beneficiary', route.url.pathname); return; }
    const escrowId = body.escrowId ?? `escrow-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

    // On-chain
    if (ctx.walletService) {
      try {
        const result = await ctx.walletService.createEscrow({
          escrowId, beneficiary,
    ...
    try {
      const envelope = await createWalletEscrowCreateEnvelope({
        ...
        depositor: body.did, beneficiary,
        amount: String(body.amount), releaseRules: body.releaseRules,
        arbiter: body.arbiter, refundRules: body.refundRules,
        expiresAt: body.expiresAt, ts: body.ts ?? Date.now(),
        nonce: body.nonce ?? 0, prev: body.prev,
      });
      ...
      if (body.autoFund !== false) {
        const fundEnvelope = await createWalletEscrowFundEnvelope({
          ...
          prev: hash, resourcePrev: hash,
        });
        await ctx.publishEvent(fundEnvelope);
      }

      created(res, {
        id: escrowId, amount: Number(body.amount), released: 0,
        remaining: Number(body.amount), status: 'active',
        releaseConditions: body.releaseRules, createdAt: body.ts ?? Date.now(),
        expiresAt: body.expiresAt, expired: false, txHash: hash,
      }, { self: `/api/v1/escrows/${escrowId}` });
    } catch { internalError(res, 'Escrow creation failed'); }
  });
    ...
    if (ctx.walletService) {
      try {
        ...
        if (result) {
          ...
          const total = (result as unknown as Record<string, unknown>).total ?? (escrows as unknown[]).length;
  ...
  r.get('/:id', async (_req, res, route) => {
    ...
    if (!ctx.eventStore) { notFound(res, `Escrow ${id} not found`); return; }
    const state = await buildWalletState(ctx.eventStore);
    const escrow = state.escrows[id];
    if (!escrow) { notFound(res, `Escrow ${id} not found`); return; }
  ...
  r.post('/:id/actions/fund', async (_req, res, route) => {
    ...
    try {
      const envelope = await createWalletEscrowFundEnvelope({
        issuer: body.did, privateKey, escrowId: id,
        amount: String(body.amount), resourcePrev: body.resourcePrev,
        ts: body.ts ?? Date.now(), nonce: body.nonce, prev: body.prev,
      });
      const hash = await ctx.publishEvent(envelope);
      ok(res, { txHash: hash, amount: Number(body.amount), status: 'broadcast', timestamp: body.ts ?? Date.now() },
        { self: `/api/v1/escrows/${id}` });
    } catch { internalError(res, 'Escrow fund failed'); }
  });
  ...
  r.post('/:id/actions/release', async (_req, res, route) => {
    ...
    try {
      const envelope = await createWalletEscrowReleaseEnvelope({
        issuer: body.did, privateKey, escrowId: id,
        amount: String(body.amount), ruleId: body.ruleId ?? 'manual',
      ...
      ok(res, { txHash: hash, amount: Number(body.amount), status: 'broadcast', timestamp: body.ts ?? Date.now() },
        { self: `/api/v1/escrows/${id}` });
    } catch { internalError(res, 'Escrow release failed'); }
  });
  ...
  r.post('/:id/actions/refund', async (_req, res, route) => {
    ...
    try {
      const envelope = await createWalletEscrowRefundEnvelope({
        issuer: body.did, privateKey, escrowId: id,
        amount: String(body.amount), reason: body.reason ?? 'manual',
        resourcePrev: body.resourcePrev,
        ts: body.ts ?? Date.now(), nonce: body.nonce, prev: body.prev,
      });
      const hash = await ctx.publishEvent(envelope);
      ok(res, { txHash: hash, amount: Number(body.amount), status: 'broadcast', timestamp: body.ts ?? Date.now() },
        { self: `/api/v1/escrows/${id}` });
    } catch { internalError(res, 'Escrow refund failed'); }
  });
  ...
  r.post('/:id/actions/expire', async (_req, res, route) => {
    ...
    if (!ctx.eventStore) { internalError(res, 'Event store unavailable'); return; }
    const state = await buildWalletState(ctx.eventStore);
    const escrow = state.escrows[id];
    if (!escrow) { notFound(res, `Escrow ${id} not found`); return; }
    ...
    if (remaining <= 0n) { badRequest(res, 'Escrow already empty', route.url.pathname); return; }
    ...
    try {
      ...
      if (action === 'release') {
        envelope = await createWalletEscrowReleaseEnvelope({
          issuer: body.did, privateKey, escrowId: id,
          amount: String(remaining), ruleId: 'expired',
        ...
        envelope = await createWalletEscrowRefundEnvelope({
          issuer: body.did, privateKey, escrowId: id,
          amount: String(remaining), reason: 'expired',
          resourcePrev: body.ruleId ?? '',
          ts: body.ts ?? Date.now(), nonce: body.nonce, prev: body.prev,
        });
      }
      const hash = await ctx.publishEvent(envelope);
      ok(res, {
        txHash: hash, amount: Number(remaining), action,
        status: 'broadcast', timestamp: body.ts ?? Date.now(), expiresAt,
      }, { self: `/api/v1/escrows/${id}` });
    } catch { internalError(res, 'Escrow expire failed'); }
  });
  ...
  r.post('/:id/actions/dispute', async (_req, res, route) => {
    ...
    if (ctx.walletService) {
      try {
        ...
        if (result) { ok(res, result, { self: `/api/v1/escrows/${id}` }); return; }
      } catch (err) { internalError(res, (err as Error).message); return; }
    }

    // No dedicated legacy envelope for escrow dispute — treat as generic action
    ok(res, { escrowId: id, status: 'disputed', timestamp: Date.now() },
      { self: `/api/v1/escrows/${id}` });
  });
  ...
  r.post('/:id/actions/resolve', async (_req, res, route) => {
    ...
    ok(res, { escrowId: id, status: 'resolved', timestamp: Date.now() },
      { self: `/api/v1/escrows/${id}` });
  });
