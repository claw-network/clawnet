...
import {
  TaskPublishSchema, TaskBidSchema, TaskBidActionSchema,
  TaskAcceptSchema, TaskDeliverSchema, TaskConfirmSchema, TaskReviewSchema,
...
export function marketsTaskRoutes(ctx: RuntimeContext): Router {
  ...
  r.post('/', async (_req, res, route) => {
    ...
    const body = v.data;
    const listingId = body.listingId ?? `task-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    try {
      const envelope = await createTaskListingPublishEnvelope({
        ...
        marketData: {
          taskType: body.taskType, task: body.task,
          timeline: body.timeline, workerRequirements: body.workerRequirements,
          bidding: body.bidding, milestones: body.milestones,
        } as never,
      ...
      created(res, { listingId, txHash: hash },
        { self: `/api/v1/markets/tasks/${listingId}` });
    } catch { internalError(res, 'Task listing publish failed'); }
  });
    ...
    if (ctx.searchMarkets) {
      try {
        const result = ctx.searchMarkets({
          markets: ['task'], page, pageSize: perPage,
  ...
  r.get('/:id', async (_req, res, route) => {
    ...
    if (ctx.marketStore) {
      try {
        ...
        if (listing) { ok(res, listing, { self: `/api/v1/markets/tasks/${id}` }); return; }
      } catch { /* fallthrough */ }
    }
  ...
  r.delete('/:id', async (_req, res, route) => {
    ...
    } catch { internalError(res, 'Task listing remove failed'); }
  });
  ...
  r.post('/:id/bids', async (_req, res, route) => {
    ...
    try {
      ...
      const envelope = await createMarketBidSubmitEnvelope({
        issuer: body.did, privateKey, bidId,
        taskId: id,
        proposal: {
      ...
      const hash = await ctx.publishEvent(envelope);
      created(res, { bidId, txHash: hash },
        { self: `/api/v1/markets/tasks/${id}/bids/${bidId}` });
    } catch { internalError(res, 'Bid submit failed'); }
  });
  ...
  r.post('/:id/bids/:bidId/actions/accept', async (_req, res, route) => {
    ...
    try {
      ...
      const e2 = await createMarketOrderCreateEnvelope({
        issuer: body.did, privateKey, listingId: id, orderId,
        marketType: 'task' as never,
        items: [] as never, pricing: {} as never,
        ts: ts + 1, nonce: ++nonce, prev: h1,
      });
      ...
      const e4 = await createWalletEscrowFundEnvelope({
        issuer: body.did, privateKey, escrowId,
        amount: '0', resourcePrev: h3,
        ts: ts + 3, nonce: ++nonce, prev: h3,
      });
      ...
      const e5 = await createMarketOrderUpdateEnvelope({
        ...
        status: 'paid',
        resourcePrev: h4,
        ts: ts + 4, nonce: ++nonce, prev: h4,
      });
      const h5 = await ctx.publishEvent(e5);

      created(res, {
        bidId: body.bidId ?? bidId, orderId, escrowId,
        bidAcceptHash: h1, orderHash: h2, escrowCreateHash: h3,
        escrowFundHash: h4, paymentHash: h5,
      }, { self: `/api/v1/markets/tasks/${id}` });
    } catch { internalError(res, 'Bid accept failed'); }
  });
  ...
  r.post('/:id/bids/:bidId/actions/reject', async (_req, res, route) => {
    ...
    try {
      ...
      const hash = await ctx.publishEvent(envelope);
      ok(res, { bidId: body.bidId ?? bidId, txHash: hash },
        { self: `/api/v1/markets/tasks/${id}` });
    } catch { internalError(res, 'Bid reject failed'); }
  });
  ...
  r.post('/:id/bids/:bidId/actions/withdraw', async (_req, res, route) => {
    ...
    try {
      ...
      ok(res, { bidId: body.bidId ?? bidId, txHash: hash },
        { self: `/api/v1/markets/tasks/${id}` });
    } catch { internalError(res, 'Bid withdraw failed'); }
  });
  ...
  r.post('/:id/actions/deliver', async (_req, res, route) => {
    ...
    if (!privateKey) { badRequest(res, 'Key unavailable', route.url.pathname); return; }
    try {
      const submissionId = body.submissionId ?? `sub-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const ts = body.ts ?? Date.now();
      ...
      const e1 = await createMarketSubmissionSubmitEnvelope({
        ...
        notes: body.notes,
        ts, nonce: body.nonce, prev: body.prev,
      });
      const h1 = await ctx.publishEvent(e1);
      ...
      const e2 = await createMarketOrderUpdateEnvelope({
        ...
        status: 'delivered',
        resourcePrev: h1,
        ts: ts + 1, nonce: body.nonce + 1, prev: h1,
      });
      const h2 = await ctx.publishEvent(e2);

      ok(res, { submissionId, submissionHash: h1, orderUpdateHash: h2 },
        { self: `/api/v1/markets/tasks/${id}` });
    } catch { internalError(res, 'Task delivery failed'); }
  });
  ...
  r.post('/:id/actions/confirm', async (_req, res, route) => {
    ...
    } catch { internalError(res, 'Task confirm failed'); }
  });
  ...
  r.post('/:id/actions/review', async (_req, res, route) => {
    ...
    } catch { internalError(res, 'Task review failed'); }
  });
