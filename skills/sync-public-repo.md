...
| **Trigger** | push to `main`（19:00-07:59 CST）、手动 `workflow_dispatch` |
| **Time gate** | 08:00-19:00 CST skip；其余时段 + 手动 dispatch 始终同步 |
| **Tag sync** | —（不同步 tag，发布在私有仓库完成） |
...
| `.github/workflows/publish-packages.yml` | 发布工作流（发布仅在私有仓库进行） |
...
1. **Time gate check**：判断当前时间（Asia/Shanghai），08:00-18:59 skip，手动 dispatch 始终放行
...
> **Note**: Tag 不再同步到公开仓库。发布（npm/PyPI/GPR）均在私有仓库 `clawnet-dev` 的 `publish-packages.yml` 工作流中完成。
